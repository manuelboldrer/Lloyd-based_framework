function [Xvis1,Yvis1,count1]= visibilitypointsNOCOH(x,y,obs,obsD,VobsD,rr,xlim,ylim,Rmax,epsi,q,dA,Rcoh,th,b,Ad, Rmaxt0)
...
%                 count1 = count1+1;
        ...
        for jj = 1:length(x)
            if q ~= jj          
                ...
                if (xcirc(j)-x(q))*cos(alpha(jj)) + (ycirc(j)-y(q))*sin(alpha(jj)) < norm([x(q)-x(jj),y(q)-y(jj)])*Rmaxt0(q)/(Rmaxt0(jj)+...
                       Rmaxt0(q)) %|| ( Ad(q,jj) ==0 && norm([xcirc(j)-x(q),ycirc(j)-y(q)]) < Rmax(q) ) 
...
%                 end
        ...
        if noCollision([x(q),y(q)]',[xcirc(j),ycirc(j)]',obs)==1 && all(vec1) && all(vec2) && all(vec)
            %       if noCollision([x,y]',[xcirc(j),ycirc(j)]',obs)==1 && (rr - (X-0.5)^2-(Y-0.5)^2)<0
            p=p+1;
            
            Xvis{1}(p) = xcirc(j);
            Yvis{1}(p) = ycirc(j);
            if norm([xcirc(j)-x(q),ycirc(j)-y(q)])>Rmaxt0(q)-0.05
                count1 = count1+1;
            end
        end
        end
