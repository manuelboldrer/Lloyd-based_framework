function [Xvis1,Yvis1]= visibilitypointsNOWALL(x,y,obs,obsD,VobsD,rr,xlim,ylim,Rmax,epsi,q,dA,Rcoh,th,b,Ad, Rmaxt0)
...
for r=0:0.05:Rmaxt0(q)
    for j = 1:length(th)
        ...
        if r < dA(q)+0.2
            ...
            if xcirc1(j) >xlim(2)
                xcirc1(j)=xlim(2);
            elseif xcirc1(j) <= xlim(1)
                xcirc1(j) = xlim(1);
            end
            if ycirc1(j) >ylim(2)
                ycirc1(j)=ylim(2);
            elseif ycirc1(j) <= ylim(1)
                ycirc1(j) = ylim(1);
            end
...
%                 end
        ...
        if  all(vec1) && coh && all(vec2) && noCollision([x(q),y(q)]',[xcirc1(j),ycirc1(j)]',obs)==1 
