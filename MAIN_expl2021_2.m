...
%% Parameters
beh             = ['1','1','1','1'];  %behavior of the agent: '1','2','3' = flocking,
% 'c' = coverage,'r' = rendezvous, %'e' = exploration. 
dx              = 0.03 ;%0.075  %space discretization
...
RmaxComm        = 8*ones(n_agents,1);  %Communication Radius  
xlim            = [0 30] ;
ylim            = [0 30] ;
...
hconeO              = zeros(2,length(time),n_agents,n_obsD);
translO             = zeros(2,length(time),n_agents,n_obsD);
alphaconeO          = zeros(length(time),n_agents,n_obsD);
collisionO          = zeros(length(time),n_agents,n_obsD);
velocities          = zeros(n_agents,2,length(time));
count               = zeros(n_obsD,n_agents);
direction1          = zeros(n_obsD,n_agents);
rot_matrix_store    = zeros(n_agents,n_obsD,2,2);
F_max               = 5  ;
egg.a               = 2.4     ;
egg.epsilon         = 1     ;
egg.b               = egg.epsilon * egg.a ;
egg.alpha           = 0     ;
egg.trasl           = 0*egg.a*0.75 ;
F_total1            = zeros(2,length(time)) ;
F_attrac            = zeros(2,length(time)) ;
